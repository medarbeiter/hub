 ...
 * MedArbeiter Theme
 *
 * Derived from the Astryx Neutral theme: brand gold #e1b025 as accent on a
 * white/warm-paper ground, warm stone neutrals, Poppins headings (wordmark
 * kinship), and warnings shifted to orange so status never impersonates the
 * brand color. The categorical OKLCH palette below is inherited unchanged.
 *
 * Original Neutral notes: a pure grayscale spine with a from-scratch
 * OKLCH-derived categorical palette. Hues are placed at evenly-spaced positions on the OKLCH wheel,
...
export const medarbeiterTheme = defineTheme({
  name: 'medarbeiter',

  // Typography: Poppins headings echo the MedArbeiter wordmark and marketing
  // site; Figtree body for long-form legibility at data density.
  ...
  tokens: {
    ...
    // =========================================================================
    // MedArbeiter: white surfaces on a barely-warm paper body — the brand's
    // white ground with the marketing site's warm cast, not a gray ladder.
    '--color-background-surface': ['#ffffff', '#28261f'],
    '--color-background-body': ['#faf8f3', '#1c1b16'],
    '--color-background-card': ['#ffffff', '#1c1b16'],
    '--color-background-popover': ['#ffffff', '#1c1b16'],
    '--color-background-muted': ['#f5f2ea', '#1c1b16'],
    // Die umgekehrte Fläche — die Meldung unten rechts ist ihr einziger
    // Verbraucher. Astryx' Vorgabe ist ein kühles Blauschwarz (#0A1317); in
    // einem Haus aus warmem Papier wäre das der einzige kalte Ton. Also die
    // Tinte dieses Themas, und auf der Gegenseite sein Papier.
    '--color-background-inverted': ['#1c1917', '#faf9f5'],

    // Accent + neutral surface tints (sit alongside backgrounds)
    // Brand gold #e1b025 carries the accent; muted stop is its pastel wash.
    '--color-accent': ['#e1b025', '#e5bc44'],
    '--color-accent-muted': ['#f7edd2', '#3a3012'],
    ...
    // Text
    '--color-text-primary': ['#1c1917', '#faf9f5'],
    '--color-text-secondary': ['#67625a', '#a9a49a'],
    '--color-text-disabled': ['#a8a29e', '#57534e'],
    // Text-grade gold is bronze: #7c5f05 clears 6:1 on white.
    '--color-text-accent': ['#7c5f05', '#eece6d'],
    '--color-on-dark': '#ffffff',
    '--color-on-light': '#171717',
    // Contrast: neutral accent is near-black (L) / near-white (D)
    // Gold is bright: dark ink on it in both modes (white on gold fails AA).
    '--color-on-accent': ['#231a02', '#231a02'],
    '--color-on-success': ['#ffffff', '#171717'],
    '--color-on-error': ['#ffffff', '#171717'],
    '--color-on-warning': '#171717',

    // Icon
    '--color-icon-accent': ['#8f6e06', '#eece6d'],
    ...
    // Warning moves to the orange family: yellow-warning would collide with
    // the brand gold that paints work segments and primary actions.
    '--color-warning': ['#6e3500', '#ffc9a2'],
    '--color-success-muted': ['#c5e5c0', '#84c9803D'],
    '--color-error-muted': ['#facecb', '#ff9e973D'],
    '--color-warning-muted': ['#fad0b5', '#ffa2583D'],

    // Border
    '--color-border': ['#1c191714', '#FFFFFF1A'],
    '--color-border-emphasized': ['#d8d2c6', '#57534e'],

    // Effects
    // Der Platzhalter trägt das Papier dieses Hauses, nicht Astryx' kaltes
    // Grau #ebebeb.
    //
    // Ein Platzhalter trägt keine Bedeutung; er darf deshalb auch nicht wie
    // Arbeit aussehen. Volles Gold (`--color-accent-muted` #f7edd2) hätte
    // genau das behauptet — ein Bildschirm voll Goldbalken sagt „hier ist
    // überall gearbeitet worden", bevor irgendetwas geladen ist. Also nur eine
    // Ahnung davon: #ece2c9 steht 1,21:1 auf dem Papiergrund #faf8f3 und damit
    // genauso deutlich wie Astryx' eigenes #ebebeb (1,24:1) — gleiche
    // Lautstärke, andere Temperatur. Ein Platzhalter ist nicht
    // bedeutungstragend (`aria-hidden`) und fällt darum nicht unter die
    // 3:1-Schwelle.
    //
    // Die Bewegung bleibt Astryx' eigenes Pulsieren, für
    // `prefers-reduced-motion` dort bereits stillgelegt — kein Wischlicht,
    // kein dritter Bewegungsmoment (siehe DESIGN.md, „Bewegung").
  ...
  components: {
    ...
    statusdot: {
      ...
      'variant:accent': {backgroundColor: 'light-dark(#e1b025, #e5bc44)'},
    },
    ...
    progressbar: {
      ...
      'variant:accent': {
        // Brand gold — progress in MedArbeiter is worked time filling up.
        '--color-accent': '#e1b025',
      },
      'variant:success': {
  ...
  // ===========================================================================
  // Auf der umgekehrten Fläche — die Aufmerksamkeitsmeldung, und sonst nichts.
  //
  // Astryx' Rechnung dafür lautet: `MediaTheme` dreht `color-scheme`, und alle
  // `light-dark()`-Paare kippen von selbst auf ihre dunkle Hälfte. Nachgemessen
  // im Browser kippen sie NICHT — eine Benutzerdefinition wird dort aufgelöst,
  // wo sie deklariert ist (am `:root`, also hell), nicht dort, wo sie benutzt
  // wird. Auf der Tinte stünde damit Stein #67625a (2,8:1) und Bronze #7c5f05
  // (2,0:1): unlesbar, und zwar genau die Sorte Fehler, gegen die
  // tests/kontrast.test.ts geschrieben ist.
  //
  // Also wird hier gesagt, was dort gilt, statt es zu erwarten. Jeder Wert ist
  // die dunkle Hälfte des Paares von oben — dieselbe Farbe, die dieses Thema im
  // Dunkelmodus ohnehin führt, nur ausdrücklich. Die Paarungen stehen als
  // Rechnung in tests/kontrast.test.ts.
  // ===========================================================================
  onDark: {
    tokens: {
      ...
      // Damit eine getönte Schaltfläche auf der Tinte überhaupt eine Tönung hat
      // und ihr Überfahren zu sehen ist: schwarz auf schwarz wäre beides nicht.
