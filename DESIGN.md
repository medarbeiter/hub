...
Die Stammdatenfreigabe ist nicht überspringbar. Ein Hinweis erklärt den Rückweg zur Verwaltung, statt Felder editierbar aussehen zu lassen, die der Mitarbeiter nicht verantwortet. Persönliche Vorgaben bleiben dagegen unter „Mein Profil" änderbar; der Name im Fuß der Seitenleiste ist der Weg dorthin. Sechs eigens erstellte Tierporträts liegen als ein lokaler 3×2-Bildbogen vor: verspielt, aber ohne Foto-Upload, biometrische Daten oder fremden Abruf. Google erscheint ausschließlich nach der MedArbeiter-Anmeldung als klar bezeichnete OAuth-Verknüpfung des bestehenden Kontos — niemals als zweite Login-Schaltfläche. Solange echtes OAuth nicht verbunden ist, sperrt der Server diesen Schritt im Produktivbetrieb.

### Icons — the Sinnbild vocabulary
**Typicons (`react-icons/ti`), and one module names every meaning: `components/sinnbilder.tsx`.** Components never name a glyph, only a meaning (`sinn="einstempeln"`), so "bearbeiten" cannot become three different pencils across the entry row, the team sheet and the employee admin. A new icon is added by adding a *meaning* — and the author sees on the way in whether that meaning already has one.

Typicons has **no weight axis** — every glyph ships in one weight, and only part of the set has an `…Outline` counterpart. The three-weight system this vocabulary used to run on is therefore replaced by a **two-value form axis**:

- **voll** — the default. A solid glyph, identical at every size. Because Typicons draws in mass rather than in stroke, even a 14 px glyph carries enough area for the 3:1 non-text floor; the weight that `bold` used to supply now sits in the fill.
- **umriss** — the same glyph as a contour, *only* for the not-selected / not-running state. This preserves the channel `fill` used to carry: solid means "running now / decided / selected", contour means the opposite.

Astryx separates `icon` from `selectedIcon` in SideNav and Tab, and that pair is what the form axis feeds. Where Typicons ships no contour the helper falls back to the solid form and the selection is carried by the nav's own highlight or the Bereichsleiste's gold underline alone — that is the case for `mitarbeiter` and `abschluss`, and it is listed in the module header.

**Two gaps the set does not fill**, recorded rather than papered over: there is no bed (Übernachtung wears the outlined roof against Feierabend's solid one) and there is no takeoff/landing pair (An- and Abreise wear a thick up and down arrow).
...
- **A trip's status wears the sign of the act that produced it**: Entwurf = pencil, eingereicht = paper plane, genehmigt = seal-check, abgelehnt = seal-warning.
- **Half and full per-diem rates are a half-filled and a filled circle** — the glyphs say what the amounts say.

Astryx's own built-in glyphs (checkbox ticks, Selector chevrons, Banner status marks) are re-registered to Typicons in `theme/icons.tsx`, so a chevron in a Selector and a chevron in the Bereichsleiste are the same shape.
