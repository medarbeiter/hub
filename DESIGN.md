...
- **umriss** — weight `bold`, deliberately **not** `regular`. The same glyph as a contour, *only* for the not-selected / not-running state. `regular` measures 16 of 256 units and lands under 0.9 px at `GROESSE.zeile` (14 px); `bold` measures 24 and lands near 1.3 px, which is the mass a filled Typicon used to carry there. Against `fill` it still reads unmistakably as a contour.
...
Astryx's own built-in glyphs (checkbox ticks, Selector chevrons, Banner status marks) are re-registered to Phosphor in `theme/icons.tsx`, all at weight `bold` for the same reason `umriss` is, so a chevron in a Selector and a chevron in the Bereichsleiste are the same shape. The three gaps Typicons left there are closed too: `moreHorizontal` is three dots (was a menu grid), `eyeSlash` a struck-through eye (was a lighter eye), `copy` a double sheet (was a clipboard). `checkDouble` is now Phosphor's actual double check instead of the ticked box that also means `genehmigen` — two things that were one picture are two again.
