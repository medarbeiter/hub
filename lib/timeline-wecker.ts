...
export type NeuesEreignis = Pick<TimelineEvent, 'id' | 'art' | 'titel' | 'beschreibung' | 'url' | 'goalId'> & {person: {id: number; name: string}};
...
export async function neueEreignisse(seit: number, jetzt = Date.now()): Promise<{jetzt: number; ereignisse: NeuesEreignis[]}> {
  ...
  for (const e of letzte.events) {
    ...
    if (erstmals.get(e.id)! > seit) neu.push({id: e.id, art: e.art, titel: e.titel, beschreibung: e.beschreibung, url: e.url, goalId: e.goalId, person: {id: e.person.id, name: e.person.name}});
  }
  gefuellt = true;
