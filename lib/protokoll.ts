...
export function beschreibeFahrzeugBeleg(belegId: number): Gegenstand | null {
  ...
  if (!row) return null;
  return {
    text: `Fahrzeugbeleg ${fmtEuro(row.betrag_cent)} vom ${fmtDate(row.datum)}`,
    betroffen: person(row.user_id),
    datum: row.datum,
    werte: {Art: row.art, Betrag: fmtEuro(row.betrag_cent), Beschreibung: row.beschreibung, Datei: row.datei_name},
  };
}
